...
import { Loader2, Search, CheckCircle, XCircle, Clock, FileText, ExternalLink } from 'lucide-react';
...
export default function PrincipalDashboard() {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const approvedTodayCount = data?.filter(d => {
        if (d.status !== 'approved' || !d.reviewed_at) return false;
        const reviewDate = new Date(d.reviewed_at);
        return reviewDate >= today;
      }).length || 0;

      setStats({ pending: pendingCount, approvedToday: approvedTodayCount, rejected: rejectedCount });
    } catch (error: any) {
  ...
  const handleAction = async (docId: string, status: 'approved' | 'rejected', targetUserId: string) => {
    ...
    try {
      ...
      // If approved, trigger blockchain record
      if (status === 'approved') {
        const formData = new FormData();
        // Since we don't have the file object here, we send a signal to the backend
        // to record the hash on the blockchain. The actual implementation would
        // fetch the file from Supabase and pass it, or just pass the file_url.
        ...
        // We create an empty file just to satisfy multer in the backend
        const emptyFile = new File([''], 'placeholder.pdf', { type: 'application/pdf' });
        formData.append('documents', emptyFile);
    ...
    try {
      const response = await fetch("/api/did/vc/issue", {
        ...
        body: JSON.stringify({
          ...
          credentialType: "MarksheetCredential",
          claims: { docId, status: "Verified", institution: "Educational Board" },
          expiryDays: 365
        })
      });
        ...
        <Card className="shadow-sm border-green-100">
          <CardContent className="p-6 flex items-center space-x-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-800">{stats.approvedToday}</h3>
            </div>
          </CardContent>
        </Card>
                      ...
                      <TableCell>
                        <a href={doc.file_url} target="_blank" rel="noopener noreferrer">
                          <Button variant="outline" size="sm" className="text-blue-600">
                            <FileText className="h-4 w-4 mr-2" /> View File <ExternalLink className="h-3 w-3 ml-1" />
                          </Button>
                        </a>
                      </TableCell>
